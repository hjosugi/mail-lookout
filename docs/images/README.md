# Screenshots

Processed screenshots used by the project README and the AppSource listing.
Personal email addresses are blurred. The raw, unblurred source captures live
in `raw/` and are **gitignored** (never committed).

| File | Use |
| --- | --- |
| `review.png` | README — the review pane (recipients + external badge, body) |
| `countdown.png` | README — the send-wait countdown |
| `smart-alert.png` | README — the Smart Alerts message on Send |
| `appsource/*-1366x768.png` | AppSource listing screenshots (1366×768, < 1 MB) |
| `../../public/assets/icon-96.png` | Microsoft Marketplace listing icon |
| `../../public/assets/icon-128.png` | Outlook high-DPI add-in icon |

Marketplace image audit, 2026-07-02:

- AppSource screenshots are 1366×768 PNG files and each is under 1 MB.
- README screenshots still match the current Japanese review, countdown, and
  Smart Alerts text.
- Store icons include the 96×96 listing icon and 128×128 Outlook high-DPI icon.
