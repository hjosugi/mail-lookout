...
ダイアログに、宛先・添付ファイル・警告・本文プレビューを表示します。
問題なければ下書きに戻り、内容を変更せずにもう一度送信します。目的は
小さなミスを防ぐことです。宛先の間違い、添付の付け忘れ、件名の空欄、
といったものです。
...
4. **二度目の送信で確認完了。** 1 回目の送信を止めます。下書きが
   変わっていなければ、次の送信を確認済みとして許可します。
...
Outlook 標準の Smart Alerts ダイアログに確認内容が表示されます。問題
なければ閉じて、下書きを変更せずにもう一度送信を押します。
...
1 回目の送信では Smart Alerts ダイアログに確認内容を表示し、送信を
中止します。下書きが変わっていなければ、次の送信を「確認済み」として
許可します。件名・本文・宛先・添付が変わった場合は、もう一度確認を
表示します。予期しないエラーが起きた場合も、ハンドラは送信を中止
します。確認なしに実メールを送ることはありません。
